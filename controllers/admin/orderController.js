...
const updateItemStatus = async (req, res) => {
    try {
      ...
      const allReturnRejected = order.orderItems.every(
        (i) => i.status === "Return Rejected"
      );
      const allReturned = order.orderItems.every((i) => i.status === "Returned");
      const anyReturnRequested = order.orderItems.some(
        (i) => i.status === "Return Requested"
      );
      const anyReturnApproved = order.orderItems.some(
...
const handleItemReturn = async (req, res) => {
  try {
      ...
      if (action === 'approve') {
          ...
          order.markModified('orderItems');//if modified nexted object
      } else {
          return res.status(400).json({ 
              success: false, 
